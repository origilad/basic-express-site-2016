...
exports.LoggedAdventure = function(req, res){
  try {
    ...
    //var emojis = [emoji.smile, emoji.crying_cat_face, emoji.stuck_out_tongue_closed_eyes, emoji.sleeping]; 
    var emojis = [emoji.smile, '1']
    ...
    for(var i = 0; i<jsonContent.users.length; i++){
       if(jsonContent.users[i].id === user){
         ...
         res.render('logged-adventure', {lat: lat, lng:lng, title: 'Logged Adventure', user: jsonContent.users[i], values: [], emojis: emojis});
       }
    }
 } catch (e) {
    next(e)
